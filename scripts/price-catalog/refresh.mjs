...
// Drop redundant variants the seed's rules exclude: Pro-tier prefixes,
...
async function main() {
  const catalogPath = process.argv[2] || "internal/pricecatalog/catalog.json";
  ...
  // wiping them. A full overwrite would drop the embedded seed prices for any
  ...
  for (const p of PROVIDERS) {
    try {
      ...
      // {} would drop the embedded seed prices for that host, breaking it
      // until the fetcher is fixed — the opposite of "preserve what we have".
      // Skip the assignment instead, so the host keeps its prior entries and
      // the report flags it.
      if (count === 0) {
