...
func TestCreateOAuthProviderEncryptsSecretAndDefaultsMappings(t *testing.T) {
	...
	plain, err := crypto.Decrypt(testMasterKey(), stored.EncryptedClientSecret)
...
func TestUpdateOAuthProviderKeepsSecretWhenOmitted(t *testing.T) {
	...
	if plain, err := crypto.Decrypt(testMasterKey(), stored.EncryptedClientSecret); err != nil || plain != "very-secret" {
	...
	if plain, err := crypto.Decrypt(testMasterKey(), stored.EncryptedClientSecret); err != nil || plain != "rotated" {
