...
func TestUpdateModelCandidateDefaultsProviderModelNameToModelNameWhenBlank(t *testing.T) {
	...
	updated, err := svc.UpdateModelCandidate(context.Background(), created.ID, UpdateCandidateInput{
...
func TestTestAndCreateCandidateDefaultsProviderModelNameToModelNameWhenBlank(t *testing.T) {
	...
	if _, err := svc.TestAndCreateCandidate(context.Background(), modelView.ID, CreateCandidateInput{
		ProviderID: provider.ID, ProviderModelName: "", InputPrice: 1, OutputPrice: 2,
		ManagementStatus: model.ModelCandidateStatusEnabled,
	}, now); err != nil {
...
func seedCandidateForRetest(t *testing.T, svc *ModelService, providerID uint, now time.Time) *CandidateView {
	t.Helper()
	modelView, err := svc.CreateModel(CreateModelInput{Name: "smart"}, now)
	if err != nil {
		t.Fatalf("CreateModel failed: %v", err)
...
func TestClassifyCapabilityResult(t *testing.T) {
	cases := []struct {
		...
		{name: "success confirms", result: TestResult{Outcome: TestSuccess}, want: boolPtr(true)},
		...
		{name: "failed validation is unknown", result: TestResult{Outcome: TestUpstreamError}, want: nil},
		{name: "a refusal is unknown", result: TestResult{Outcome: TestUpstreamError}, want: nil},
		{name: "rate limited is unknown", result: TestResult{Outcome: TestRateLimited}, want: nil},
		{name: "auth failure is unknown", result: TestResult{Outcome: TestAuthFailed}, want: nil},
		{name: "wrong model name is unknown", result: TestResult{Outcome: TestModelNotFound}, want: nil},
		{name: "quota exhaustion is unknown", result: TestResult{Outcome: TestQuotaUnavailable}, want: nil},
		{name: "unreachable is unknown", result: TestResult{Outcome: TestUnreachable}, want: nil},
		{name: "an uncertifiable protocol is unknown", result: TestResult{Outcome: TestVerificationUnsupported}, want: nil},
	}
...
func TestRetestModelCandidateKeepsEarnedCapabilityWhenProbeIsUnconfirmed(t *testing.T) {
	...
	client.perTestType = map[string]fakeTargetResponse{
		"basic":            {result: TestResult{Outcome: TestSuccess}},
		"streaming":        {result: TestResult{Outcome: TestRateLimited}},
		"function_calling": {result: TestResult{Outcome: TestUpstreamError}},
	}
	updated, err := svc.RetestModelCandidate(context.Background(), candidate.ID, now)
	if err != nil {
		t.Fatalf("RetestModelCandidate failed: %v", err)
...
func TestRetestModelCandidateKeepsVerificationWhenBasicProbeIsInconclusive(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "rate limited", result: TestResult{Outcome: TestRateLimited}},
		{name: "unreachable", result: TestResult{Outcome: TestUnreachable}},
		{name: "upstream 502", result: TestResult{Outcome: TestUpstreamError}},
		{name: "out of quota", result: TestResult{Outcome: TestQuotaUnavailable}},
		{name: "bad credential", result: TestResult{Outcome: TestAuthFailed}},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestTestAndCreateCandidateDoesNotPersistWhenEnableRequestedAndBasicFails(t *testing.T) {
	...
	client.result = TestResult{Outcome: TestModelNotFound}
	result, err := svc.TestAndCreateCandidate(context.Background(), modelView.ID, CreateCandidateInput{
		ProviderID: provider.ID, ProviderModelName: "gpt-4o", InputPrice: 1, OutputPrice: 2,
		ManagementStatus: model.ModelCandidateStatusEnabled,
	}, now)
	if err != nil {
...
func TestUpdateModelCandidate(t *testing.T) {
	...
	updated, err := svc.UpdateModelCandidate(context.Background(), candidate.ID, UpdateCandidateInput{
...
func TestUpdateModelCandidateResetsVerificationWhenModelNameChanges(t *testing.T) {
	...
	seedVerified()
	if _, err := svc.UpdateModelCandidate(context.Background(), candidate.ID, UpdateCandidateInput{
		ProviderModelName: "gpt-4o-mini", InputPrice: 1, OutputPrice: 2,
	}, now); err != nil {
...
// boolPtr builds a tri-state capability verdict.
func boolPtr(v bool) *bool { return &v }

// enableStatus / disableStatus build the pointer UpdateCandidateInput expects.
...
func seedEnabledCandidate(t *testing.T, svc *ModelService, client *fakeProviderClient, providerID uint, now time.Time) *CandidateView {
	t.Helper()
	client.result = TestResult{Outcome: TestSuccess}
	modelView, err := svc.CreateModel(CreateModelInput{Name: "smart"}, now)
	if err != nil {
...
func TestUpdateModelCandidateReturnsNotFoundForUnknownID(t *testing.T) {
	svc, _, _ := newTestModelService(t)
	_, err := svc.UpdateModelCandidate(context.Background(), 999999, UpdateCandidateInput{ProviderModelName: "gpt-4o"}, time.Now().UTC())
	if !errors.Is(err, errcode.ErrModelCandidateNotFound) {
		t.Fatalf("expected ErrModelCandidateNotFound, got %v", err)
	}
}
...
func TestTestAndCreateCandidateReturnsNotFoundForUnknownProvider(t *testing.T) {
	...
	_, err = svc.TestAndCreateCandidate(context.Background(), modelView.ID, CreateCandidateInput{
...
func TestUpdateModelCandidateErrorsWhenUpdateFailsForNonUniqueReason(t *testing.T) {
	...
	if _, err := svc.UpdateModelCandidate(context.Background(), candidate.ID, UpdateCandidateInput{ProviderModelName: "gpt-4o-2"}, now); err == nil {
...
func TestUpdateModelCandidateErrorsWhenReloadFails(t *testing.T) {
	...
	if _, err := svc.UpdateModelCandidate(context.Background(), candidate.ID, UpdateCandidateInput{}, now); err == nil {
		t.Fatalf("expected an error when the model_candidates table is missing")
	}
}
